...
class ScreeningForm extends Screening
{
    ...
    public function rules()
    {
        return [
            ...
            ['start_time', 'required', 'on' => ['create', 'update'], 'message' => 'START_TIME_REQUIRED'],
            ['start_time', 'date', 'format' => 'php:Y-m-d H:i:s', 'message' => 'INVALID_START_TIME'],

            ['end_time', 'required', 'on' => ['create', 'update'], 'message' => 'END_TIME_REQUIRED'],
            ['end_time', 'date', 'format' => 'php:Y-m-d H:i:s', 'message' => 'INVALID_END_TIME'],
            
            // Custom validator for time range and duration check
            [['start_time', 'end_time'], 'validateTimeRange', 'on' => ['create', 'update']],
            ...
            ['score', 'integer', 'min' => 0, 'max' => 100, 'message' => 'INVALID_SCORE_TYPE', 'on' => ['review']],
    ...
    public function validateTimeRange($attribute, $params)
    {
        if (!$this->hasErrors()) {
            ...
            // Check if duration is reasonable (at least 1 minute, max 12 hours)
            $durationSeconds = $endTime - $startTime;
            $durationMinutes = $durationSeconds / 60;

            if ($durationMinutes < 1) {
                $this->addError('end_time', 'Screening duration must be at least 1 minute');
            } elseif ($durationMinutes > 720) {
                $this->addError('end_time', 'Screening duration must not exceed 12 hours');
            }
        }
    }
